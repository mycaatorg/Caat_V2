...
| Metric | Target |
|---|---|
| Free-to-Pro conversion rate | ≥ 8% within 90 days of launch |
| AI recommendations accepted by user (bookmarked) | ≥ 40% of recommendations shown |
| Community posts created per active user per month | ≥ 2 |
| AI essay / resume review sessions per Pro user per month | ≥ 3 |
| Retention: users returning within 7 days | ≥ 55% |
...
- Moderate-to-high technical literacy (V2 users are more engaged than V1 casual visitors)
- Active on social platforms (comfortable with feed/post interactions)
...
| 6 | University & Major Recommendations | Free (basic) / Pro (detailed AI) |
...
- As a Pro student, I want the AI to flag missing or weak sections (e.g. no leadership roles, vague descriptions) so I know what to add
...
- As a Pro student, I want to ask ChatCAAT questions like "which of my bookmarked schools has the strongest computer science program?" and get a contextual answer based on my own data
- As a Pro student, I want ChatCAAT to suggest next steps when I describe where I'm stuck so I don't lose momentum
- As a Pro student, I want ChatCAAT to explain college admissions concepts (e.g. what is demonstrated interest?) in plain language
- As a Pro student, I want my chat history to persist between sessions so I can refer back to previous advice
- As a Pro student, I want ChatCAAT to proactively surface reminders (e.g. "your Early Decision deadline for X is in 3 weeks") so I don't miss dates I've added to my roadmap
...
- As a Pro student, I want to input my test scores (SAT / ACT / IB / A-Level etc.) as part of the analysis so recommendations are calibrated to my academic profile
- As a Pro student, I want the AI to rank recommended universities by fit score and explain why each was recommended so I can make informed decisions
- As a Pro student, I want the AI to recommend majors that align with my stated interests and extracurricular activities so I discover options I hadn't considered
- As a Pro student, I want to refresh my recommendations whenever I update my profile, test scores, or extracurriculars so the output stays current
- As a Pro student, I want to bookmark AI-recommended universities and majors directly from the results page so I can add them to my research list in one click
...
- As a free student, I want to see a teaser of AI recommendations (e.g. two blurred-out results) so I understand the feature's value before upgrading

### 5.5 University and Major Recommendations (Free Tier — Interest-Based)
...
- As a free student, I want to understand why a university or major was recommended so the suggestion feels relevant rather than random
...
- As a student, I want to see a scrollable feed of posts from other CAAT users so I can learn from peers going through the same process
- As a student, I want to filter the feed by topic (Application Results, Essays, Test Scores, Extracurriculars, Advice, Scholarships) so I find posts relevant to where I am in the process
- As a student, I want to see the most liked and most recent posts so I can find both timely and valuable content
- As a student, I want the feed to load quickly and paginate smoothly so browsing feels effortless

#### Creating Posts

- As a student, I want to create a text post so I can share advice, ask questions, or celebrate a result
- As a student, I want to attach my resume (as a PDF or a CAAT resume link) to a post so others can see the document that got me in
- As a student, I want to tag my post with a topic (Application Results, Essays, Scholarships, etc.) so it reaches the right audience
- As a student, I want to specify which university or major my post is about so the content is discoverable by students researching those schools
- As a student, I want to share my application outcomes (Accepted / Waitlisted / Rejected) in a structured result card format so the data is clear at a glance
- As a student, I want to share my test scores (SAT / ACT / IB) in a structured card so others know the academic context behind my result
- As a student, I want to edit or delete my posts after publishing so I can correct mistakes

#### Engagement

- As a student, I want to like a post so I can signal its value to others
- As a student, I want to comment on a post so I can ask follow-up questions or add context
...
#### Profiles in Communities

- As a student, I want my community profile to show my graduation year, intended intake, and (optionally) my bookmarked countries and majors so others know my context
- As a student, I want to choose what information from my CAAT profile is visible on my community profile so I control my privacy
- As a student, I want to follow other users so their posts appear at the top of my feed
- As a student, I want to report posts that violate community guidelines so the feed stays safe and constructive

#### Moderation

- As a student, I want posts to be screened against community guidelines (no personal attacks, no spam) so the feed stays high quality
- As the platform, posts flagged by multiple users should be hidden pending review

### 5.7 CAAT Pro — Subscription Management

- As a student, I want to see a clear comparison of Free vs Pro features on an upgrade page so I can make an informed decision
- As a student, I want to subscribe to CAAT Pro using a card or digital wallet so the process is quick
- As a Pro student, I want to manage my subscription (view billing date, cancel, update payment method) from my account settings
- As a Pro student, I want to receive a receipt after each billing cycle so I have a record
- As a student who cancels Pro, I want to retain access until the end of my paid period so I don't lose immediate access
- As a student, I want to see a clear indication throughout the app of which features require Pro so I'm never surprised by a paywall mid-task
...
- Returns feedback in three categories: Structure & Flow, Clarity & Language, Authenticity & Voice
...
- Has read access to the user's profile, bookmarked schools, majors, scholarships, essay drafts, and roadmap status at the time of each message
...
- Ingests: GPA (if provided), test scores (if provided), bookmarked majors, bookmarked countries, extracurricular categories and descriptions, stated interests from profile
...
- Runs server-side on data from the user's existing bookmarks (no AI call)
...
- Posts support: plain text (max 2000 characters), topic tag (required), optional university/major tag, optional result card, optional test score card, optional attached CAAT resume link
...
- Following: users can follow/unfollow; a "Following" feed tab shows only posts from followed users
...
- Community profiles are visible only to authenticated CAAT users (not public internet)
...
- Community posts are filtered for profanity and personal data (e.g. full name + school combination) on submission
...
- Next.js (App Router) + TypeScript — same stack as V2 baseline
...
- Existing Supabase (PostgreSQL) database extended with new tables for: community posts, comments, likes, saves, follows, notifications, AI feedback records, chat history, subscription records
- AI features powered by the Anthropic Claude API (claude-sonnet-4-6 or newer)
...
- Vercel (frontend) + Supabase (database + auth) — unchanged
...
| Feature | Free | CAAT Pro |
|---|---|---|
| Dashboard, profile, roadmap | ✓ | ✓ |
| Schools, majors, scholarships research | ✓ | ✓ |
| Essay editor (write & save) | ✓ | ✓ |
| Resume builder (create & download) | ✓ | ✓ |
| Bookmarks | ✓ | ✓ |
